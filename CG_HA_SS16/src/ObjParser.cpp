...
ObjModel *ObjParser::loadModelFromFile(const char* filename) {
    this->reset();
    ...
    while (inStream.peek() != -1) {
        ...
        if (strncmp(token, "mtllib", 6) == 0) {
            ...
            this->loadMaterial((filepath + std::string(token)).c_str(), materialMap);
        }
...
void ObjParser::loadMaterial(const char* filename, std::map<std::string, unsigned int> &materialMap) {
    Material material;
